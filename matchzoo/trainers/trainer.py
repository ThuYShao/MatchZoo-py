...
class Trainer:
        ...
        with tqdm(enumerate(self._trainloader), total=num_batch,
            ...
            for step, (inputs, target) in pbar:
                ...
                if self._iteration % self._validate_interval == 0:
                    ...
                    result1 = self.evaluate(self._testloader, True, str(epoch))

                    if self._verbose:
                        pbar.write('  Validation: ' + ' - '.join(
                            f'{k}: {round(v, 4)}' for k, v in result1.items()))
                        pbar.write('  Validation: ' + ' - '.join(
    ...
    def save_model(self,name):
        """Save the model."""
        checkpoint = self._save_dir.joinpath(name+'model.pt')
